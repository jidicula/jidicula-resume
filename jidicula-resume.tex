...
\descript{Actions Platform, GitHub}
...
\vspace{\topsep} % Hacky fix for awkward extra vertical space
\begin{tightemize}
\item Guided client teams in migrating their application workloads out of government data centres to Microsoft Azure effectively.
\end{tightemize}
\sectionsep{}
...
\vspace{\topsep} % Hacky fix for awkward extra vertical space
\begin{tightemize}
\item Developed cloud landing zones using \textbf{Terraform} and \textbf{Go} infrastructure-as-code with \textbf{Azure}.
\end{tightemize}
\sectionsep{}
...
\begin{tightemize}
\item Go package (\href{https://github.com/jidicula/go-gamco}{\bf github.com/jidicula/go-gamco}) that wraps GAMCO's API for their closed-end funds (\href{https://gabelli.com/funds/closed_ends}{gabelli.com/funds/closed\_ends}).
\item CLI tool that uses the above Go module to fetch the latest net asset value of each GAMCO closed-end fund, compare it with the stock's latest price on Yahoo Finance, and output a list of funds with a NAV/Price difference of $\ge$10\%.
